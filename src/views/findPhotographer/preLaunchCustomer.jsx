...
const PreLaunchCustomer = () => {
  ...
  const validate = () => {
    ...
    if (!form.mobile_number.trim()) next.mobile_number = 'Mobile number is required'
    else if (!/^\d{7,15}$/.test(form.mobile_number.trim())) next.mobile_number = 'Enter a valid mobile number'
    if (!form.city.trim()) next.city = 'City / event location is required'
    if (!form.event_type) next.event_type = 'Please select an event type'
    if (!form.event_date) next.event_date = 'Please select an event date'
    if (!form.requirement) next.requirement = 'Please select your requirement'
    setErrors(next)
      ...
      <div className="w-full max-w-xl mx-auto py-8 px-4">
        ...
        <div className="views-card" style={{ marginTop: 0 }}>
          ...
          <form onSubmit={handleSubmit} noValidate>
            ...
            {/* Mobile Number */}
            <div className="su-field" style={{ marginBottom: FIELD_SPACING }}>
              <label style={{ display: 'block', marginBottom: 8 }}>Mobile Number <sup style={{ color: '#ef4444' }}>*</sup></label>
            ...
            {/* City */}
            <div className="su-field" style={{ marginBottom: FIELD_SPACING }}>
              <label style={{ display: 'block', marginBottom: 8 }}>City / Event Location <sup style={{ color: '#ef4444' }}>*</sup></label>
              <div className="relative flex items-center">
            ...
            {/* Event Type */}
            <div className="su-field" style={{ marginBottom: FIELD_SPACING }}>
              <label style={{ display: 'block', marginBottom: 8 }}>Event Type <sup style={{ color: '#ef4444' }}>*</sup></label>
              <div className="relative flex items-center">
                <FiCalendar size={17} className="absolute left-3.5 text-[#f5a623] pointer-events-none z-10" />
                <select
                  value={form.event_type}
                  onChange={handleChange('event_type')}
                  style={{ paddingLeft: '38px', ...(errors.event_type ? { borderColor: '#ef4444' } : {}) }}
                >
              ...
              {errors.event_type && <p className="su-error" style={{ marginTop: 6 }}>{errors.event_type}</p>}
            </div>

            {/* Event Date */}
            <div className="su-field" style={{ marginBottom: FIELD_SPACING }}>
              <label style={{ display: 'block', marginBottom: 8 }}>Event Date <sup style={{ color: '#ef4444' }}>*</sup></label>
              <div className="relative flex items-center">
                <FiClock size={17} className="absolute left-3.5 text-[#f5a623] pointer-events-none z-10" />
                <input
                  type="date"
                  min={new Date().toISOString().split('T')[0]}
                  value={form.event_date}
                  onChange={handleChange('event_date')}
                  style={{ paddingLeft: '38px', ...(errors.event_date ? { borderColor: '#ef4444' } : {}) }}
                />
              </div>
              {errors.event_date && <p className="su-error" style={{ marginTop: 6 }}>{errors.event_date}</p>}
            </div>

            {/* Requirement */}
            <div className="su-field" style={{ marginBottom: FIELD_SPACING }}>
              <label style={{ display: 'block', marginBottom: 8 }}>Requirement <sup style={{ color: '#ef4444' }}>*</sup></label>
              <div className="relative flex items-center">
                <FiVideo size={17} className="absolute left-3.5 text-[#f5a623] pointer-events-none z-10" />
                <select
                  value={form.requirement}
                  onChange={handleChange('requirement')}
                  style={{ paddingLeft: '38px', ...(errors.requirement ? { borderColor: '#ef4444' } : {}) }}
                >
              ...
              {errors.requirement && <p className="su-error" style={{ marginTop: 6 }}>{errors.requirement}</p>}
            </div>
            ...
            <div className="flex items-center justify-center gap-1.5 text-xs text-gray-400" style={{ marginTop: 14 }}>
              ...
              <span>We will connect you with the best photographers.</span>
            </div>
          </form>
        </div>
      </div>
